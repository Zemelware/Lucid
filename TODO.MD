...
* \[ \] **Init:** Next.js 15, Tailwind, Lucide-React.  
* \[ \] **Dependencies:** Install @openrouter/sdk, framer-motion, zustand.  
* \[ \] **Env:** Set up .env.local with OPENROUTER\_API\_KEY and ELEVENLABS\_API\_KEY.  
* \[ \] **Layout:** Build a full-screen image container.  
* \[ \] **Animation:** Implement the "Breathing" CSS animation (slow scale up/down loop, 15s duration).

## **Phase 2: The Dream Logic (OpenRouter SDK)**

* \[ \] **Client:** Set up the OpenRouter client in lib/openrouter.ts:  
  const client \= new OpenRouter({ apiKey: ... });  
* \[ \] **API:** Build /api/analyze-scene.  
...
* \[ \] **Implementation:** Use client.chat.send() with the model google/gemini-3-flash.

## **Phase 3: The Audio Layer (ElevenLabs)**

* \[ \] **Voice API:** Build /api/generate-voice. 
* \[ \] **SFX API:** Build /api/generate-sfx.  
* \[ \] **Optimization:** Ensure the browser pre-loads these audio blobs so playback is instant.
